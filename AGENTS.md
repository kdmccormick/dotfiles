...
* If Kyle asks you to begin tracking config for a new program, try to first
  ...
  names (e.g. "config") rename it to something helpful (e.g. "config.myapp").
...
* Commit each logical change as a unit (git-commit it). If git state is not
  clean, refuse to make any file changes and tell Kyle to git commit before
  proceeding.
